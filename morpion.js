class Morpion {
	...
	getArrayHistory(zone) {
		let array = [];
		array.push(zone);
		array.push(this.numofturns);
		console.log(array);
		console.log(this.numofturns);
		// array = localStorage.setItem("zone", zone);
		document.getElementById("undo").addEventListener("onclick", function () {
			console.log("hello");
		});
	}

	undo = () => {
		// getArrayHistory(array);
		console.log("hello");
	...
	playerTurn = (x, y) => {
		...
		if (!this.fillGrid(x, y, this.player)) return alert("La case n'est pas vide");
